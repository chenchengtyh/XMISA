...
public class ReaderHelper {
	 ...
	 * @param btCmd
	 *            命令类型(用于指定类型的刷新)
	 * @param curOperateTagISO18000Buffer
	 *            当前标签数据
	 */
	...
	private void processGetFrequencyRegion(MessageTran msgTran) {
		...
		if (btAryData.length == 0x03) {
			...
			m_curReaderSetting.nUserDefineStartFrequency = (btAryData[3] & 0xFF) * 256 * 256 + (btAryData[4] & 0xFF)
					* 256 + (btAryData[5] & 0xFF);
			refreshReaderSetting(btCmd, m_curReaderSetting);
			writeLog(strCmd, ERROR.SUCCESS);
			return;
		} else if (btAryData.length == 0x01) {
	...
	private void processInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x09) {
			...
			int nTotalRead = (btAryData[5] & 0xFF) * 256 * 256 * 256 + (btAryData[6] & 0xFF) * 256 * 256
	...
	private void processGetAccessEpcMatch(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			if (btAryData[0] == 0x00) {
				m_curOperateTagBuffer.strAccessEpcMatch = StringTool.byteArrayToString(btAryData, 2,
						btAryData[1] & 0xFF);
	...
	private void processRealTimeInventory(MessageTran msgTran) {
		Log.e("NTI", "ReaderHelper----------1.processRealTimeInventory()");
		byte btCmd = msgTran.getCmd();
		byte[] btAryData = msgTran.getAryData();
		String strCmd = CMD.format(btCmd);
		String strErrorCode = "";
		...
		if (btAryData.length == 0x01) { // 本次获取因为错误结束
			...
			m_curInventoryBuffer.nDataCount = (btAryData[3] & 0xFF) * 256 * 256 * 256 + (btAryData[4] & 0xFF) * 256
					* 256 + (btAryData[5] & 0xFF) * 256 + (btAryData[6] & 0xFF);
	...
	private void processFastSwitchInventory(MessageTran msgTran) {
		Log.e("LXM", "ReaderHelper----------2.processFastSwitchInventory()");
		byte btCmd = msgTran.getCmd();
		byte[] btAryData = msgTran.getAryData();
		String strCmd = CMD.format(btCmd);
		String strErrorCode = "";
	...
	private void processISO180006BInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			if (findIndex == null) {
				...
				m_curOperateTagISO18000Buffer.dtIndexMap
						.put(strUID, m_curOperateTagISO18000Buffer.lsTagList.size() - 1);
			} else {
	...
	private void processGetInventoryBuffer(MessageTran msgTran) {
		Log.e("LXM", "ReaderHelper----------3.processGetInventoryBuffer()");
		byte btCmd = msgTran.getCmd();
		byte[] btAryData = msgTran.getAryData();
		String strCmd = CMD.format(btCmd);
		String strErrorCode = "";
	...
	private void runLoopInventroy() {
		// 校验盘存是否所有天线均完成
		if (m_curInventoryBuffer.nIndexAntenna < m_curInventoryBuffer.lAntenna.size() - 1
	...
	private void runLoopFastSwitch() {
		if (m_curInventoryBuffer.bLoopInventory) {
			mReader.fastSwitchAntInventory(m_curReaderSetting.btReadId, m_curInventoryBuffer.btA,
					m_curInventoryBuffer.btStayA, m_curInventoryBuffer.btB, m_curInventoryBuffer.btStayB,
					m_curInventoryBuffer.btC, m_curInventoryBuffer.btStayC, m_curInventoryBuffer.btD,
					m_curInventoryBuffer.btStayD, m_curInventoryBuffer.btInterval, m_curInventoryBuffer.btFastRepeat);
		}
	}
}
